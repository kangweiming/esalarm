 ...
 * 
 * 这个类和spring-security.xml的配置存在一个即可，作用都是一样的，但是必须都得在web.xml里配置filter
 * 如果屏蔽这个类，注释掉注解即可
 * 
 * //@Configuration  
 * //@EnableWebSecurity
 * 
 * 此类中用户及密码采用的是inMemory方式
 * 
 * @author kwm
 * @date 2017年2月13日 下午2:13:13 
 * @version V1.0 
 * 
*/
//@Configuration  
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {  
    ...
    public void configureGlobalSecurity(AuthenticationManagerBuilder auth) throws Exception {  
        auth.inMemoryAuthentication().withUser("kwm").password("123456").roles("USER");  
