...
//DOM end
	...
	//alert("please import language/cn.js in head, any problem please contact with Justin");
		}
...
YUD.setCookie = function(name,value,expiry,path,domain,secure){
...
YUD.deleteCookie =function(name){
...
YL.isEnglish=function(sValue){var myReg = /[^\x00-\x80]/;
	if(myReg.test(sValue)){
	return false;}else{
		return true;
	}
	} // ascii 0 to 128
YL.isAscii=function(sValue){var myReg = /[^\x00-\xFF]/;
	if(myReg.test(sValue)){
		return false
		}else{
			return true;
			}} // ascii 0 to 255
function get(el){return YUD.get(el);}
